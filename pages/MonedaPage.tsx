...
export default function MonedaPage() {
  ...
  const callCostsApi = async (action: string, payload: Record<string, any> = {}) => {
    if (isDemoAccount) {
      // Demo: sin persistencia real, devolver lo mismo que se envió
      if (action === "costs-load") return { costSettings: null, costSettingsUpdatedAt: null };
      if (action === "costs-save-settings") {
        return { costSettings: payload.settings || null, costSettingsUpdatedAt: new Date().toISOString() };
      }
    }
