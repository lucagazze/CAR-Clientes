...
async function main() {
  ...
  const msgId = "aWdfZDo5MjU1NzA3NzA2NDkyODY6R1BTOjE3ODQxNDU0MDAxNDk3ODA0OjE3ODQxNDM4ODI2OTIyMzU4OnNlbmQ6MTc4MDE0NDI2NTgwMTEzNDY=";
  
  console.log('Querying message details for ID:', msgId);
  try {
    const res = await fetch(`https://graph.facebook.com/v21.0/${msgId}?fields=id,message,from,created_time,shares,attachments&access_token=${pageToken}`).then(r => r.json());
