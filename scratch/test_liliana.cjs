...
async function main() {
  ...
  console.log('Querying Liliana Sueños IG Conversations...');
  const igRes = await fetch(`https://graph.facebook.com/v21.0/${LILIANA_PAGE_ID}/conversations?platform=instagram&access_token=${pageToken}&fields=id,participants,unread_count,updated_time&limit=5`).then(r => r.json());
  if (igRes.error) {
    console.error('❌ Error:', igRes.error);
  } else {
    console.log(`✅ Liliana Sueños IG Conversations: ${igRes.data?.length || 0}`);
