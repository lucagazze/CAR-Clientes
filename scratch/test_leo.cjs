...
async function main() {
  ...
  if (!page) {
    console.log('❌ Leo page not found in accounts.');
    console.log('Available pages for this token:', (accountsRes.data || []).map(p => `${p.name} (${p.id})`));
    return;
  }
  const pageToken = page.access_token;
  
  console.log('Querying Librería Mayorista Leo IG Conversations...');
  const igRes = await fetch(`https://graph.facebook.com/v21.0/${LEO_PAGE_ID}/conversations?platform=instagram&access_token=${pageToken}&fields=id,participants,unread_count,updated_time&limit=5`).then(r => r.json());
  if (igRes.error) {
    console.error('❌ Error:', igRes.error);
  } else {
    console.log(`✅ Leo IG Conversations: ${igRes.data?.length || 0}`);
